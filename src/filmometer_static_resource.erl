...
-module(filmometer_static_resource).
-export([init/1]).
...
content_types_provided(ReqData, Context) ->
  ...
  {[{CT, provide_content}], ReqData, Context#context{metadata = [{'content-type', CT} | Context#context.metadata]}}.
...
content_types_accepted(ReqData, Context) ->
  ...
  {[{MT, accept_content}], ReqData, Context#context{metadata = [{'content-type', MT} | Context#context.metadata]}}.
...
delete_resource(ReqData, Context) ->
  case file:delete(file_path(Context, wrq:disp_path(ReqData))) of
...
generate_etag(ReqData, Context) ->
  case maybe_fetch_object(Context, wrq:disp_path(ReqData)) of
    {true, BodyContext} ->
      ...
      {ETag, ReqData,
        BodyContext#context{metadata = [{etag, ETag} | BodyContext#context.metadata]}};
    _ ->
